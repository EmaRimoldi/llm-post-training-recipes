...
MODEL_NAME = "VinceEPFL/mcqa_matmlmed"
MAX_SEQ_LENGTH = 2048
CONTRASTIVE_MARGIN = 0.5
NUM_INCORRECT_OPTIONS = 3 # Standardize to 3 incorrect options
DATASET_NAME_SCIQ = 'allenai/sciq'
DATA_FILE_PATH_MATHQA = str(PROJECT_ROOT / "data" / "mathqa" / "train.json")
#DATA_CODING_MCQA = str(PROJECT_ROOT / "data" / "tuandunghcmut_coding-mcq-reasoning")
DATA_GPQA = str(PROJECT_ROOT / "data" / "gpqa")


# Update output path to reflect combined data and QLoRA status
output_suffix = "qlora" if USE_QLORA else "full"
OUTPUT_PATH = str(SCRIPT_DIR / f"qwen_mcqa_contrastive_{output_suffix}")


# --- Logging and GPU Setup ---
log_file = os.path.join(OUTPUT_PATH, f'output_contrastive_{output_suffix}.log')
...
def transform_distractor_data_to_common_format(examples):
    ...
    for i in range(num_examples_in_batch):
        ...
        new_ids.append(f"sciq_or_gpqa_train_{examples.get('id',[i]*num_examples_in_batch)[i]}" if 'id' in examples else f"sciq_genid_{random.randint(0,1000000)}_{i}") # Create unique ID
...
logger.info(f"GPQA: Filtered {original_gpqa_count - len(processed_gpqa_interim)} invalid examples after initial trasnformation. Remaining: {len(processed_gpqa_interim)}")

# --- Combine and Standardize Datasets ---
combined_dataset_interim = concatenate_datasets([processed_mathqa_interim, processed_sciq_interim, processed_gpqa_interim])
...
processed_data_save_path = "./data/qlora_final_raw_data/"
...
processed_data_save_path = "./data/qlora_final_data/"
logging.info(f"Saving processed datasets to disk at {processed_data_save_path}...")
os.makedirs(processed_data_save_path, exist_ok=True)
...
class ContrastiveTrainer(Trainer):
    ...
    def _get_option_nll(self, model, input_ids, attention_mask, labels, prompt_lengths):
        if input_ids.nelement() == 0: # Handle empty input tensor (e.g. if a negative set was empty)
            # Fix: Get device from input_ids or use next(model.parameters()).device as fallback
            device = input_ids.device if input_ids.numel() > 0 else next(model.parameters()).device
        ...
        for i in range(input_ids.size(0)): # Iterate over items in this specific (positive or one negative) batch
            ...
            if option_token_indices.sum() == 0:
                ...
                # Fix: Use input_ids device instead of model.device
                batch_option_nll.append(torch.tensor(float('inf'), device=input_ids.device))
                continue
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if positive_input_ids.nelement() == 0: # No positive examples in batch
            ...
            return torch.tensor(0.0, device=device, requires_grad=True) if not return_outputs else (torch.tensor(0.0, device=device), None)
        ...
        final_loss = total_loss / num_valid_contrastive_pairs if num_valid_contrastive_pairs > 0 else torch.tensor(0.0, device=next(model.parameters()).device, requires_grad=True)
...
class ProgressCallback(TrainerCallback):
    def on_log(self, args, state, control, model=None, logs=None, **kwargs):
        if state.is_local_process_zero:
            ...
            logger.info(f"Step {current_step}/{max_steps} ({progress_pct:.1f}%) | "
                       f"Epoch {current_epoch:.2f}/{max_epochs} | "
...
training_args_dict = {
    ...
    "per_device_eval_batch_size": 8,   # Similarly for eval
    "gradient_accumulation_steps": 8, # Effective train batch size = 4 * 8 = 32
    "logging_strategy": "steps",
    "logging_steps": 1, # Log more frequently for smaller datasets
    "save_strategy": "epoch",
    "save_total_limit": 2,
