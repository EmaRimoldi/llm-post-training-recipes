...
PUSH_MODEL_REPO_NAME = os.getenv("HF_MODEL_REPO_NAME", "rag-mcqa-model")
# ----------------------------------------
...
api = HfApi()
full_push_repo_id = f"{USERNAME}/{PUSH_MODEL_REPO_NAME}"

logger.info(f"Attempting to create/check Hugging Face repository: {full_push_repo_id} (type: model, private: False)")
try:
    # Use repo_id directly for user repositories, no 'organization' argument needed
    api.create_repo(repo_id=full_push_repo_id, private=False, repo_type="model", token=HF_TOKEN)
    logger.info(f"Repository '{full_push_repo_id}' created successfully!")
except Exception as e:
    if "You already have a model named" in str(e) or "RepositoryAlreadyExists" in str(e):
        logger.warning(f"Repository '{full_push_repo_id}' already exists. Proceeding with the upload.")
    else:
        logger.error(f"Error creating repository '{full_push_repo_id}': {e}", exc_info=True)
        sys.exit(1) # Exit if repository creation fails for other reasons

logger.info(f"Starting upload of the fine-tuned model to Hugging Face repo: {full_push_repo_id} from '{final_save_path}'...")
start_upload_time = time.time()
try:
    # Use the push_to_hub method of the Trainer itself, which also pushes the tokenizer
    # Alternatively, you could use model.push_to_hub() and tokenizer.push_to_hub()
    trainer.push_to_hub(
        repo_id=full_push_repo_id,
        token=HF_TOKEN,
        commit_message=f"Fine-tuned model for MCQA using {MODEL_NAME}"
    )
    
    upload_time = time.time() - start_upload_time
    logger.info(f"✔ Model and tokenizer uploaded successfully to Hugging Face repo: {full_push_repo_id}. Took {upload_time:.2f} seconds.")
except Exception as e:
    logger.error(f"Error pushing fine-tuned model to Hugging Face Hub: {e}", exc_info=True)
    # Don't exit here, as local saving might have succeeded even if push failed

# --- Optional: Clean up local save path ---
logger.info(f"Cleaning up local model save directory: {final_save_path}...")
try:
    shutil.rmtree(final_save_path)
    logger.info("Local model directory cleaned up successfully.")
except Exception as e:
    logger.warning(f"Could not remove local model directory '{final_save_path}': {e}")

logger.info("Final script execution complete. Your fine-tuned model is now available on Hugging Face Hub!")
